...
class StudentOperation extends Controller
{
    ...
    public function exam(){
        ...
        return view('student.exam', ['exams' => $student_info]);
    }    
    ...
    public function join_exam($id){
        ...
        $exam = Oex_exam_master::find($id); // Correct model name
        if (!$exam) {
            return redirect()->route('exam')->with('error', 'Ujian tidak ditemukan');
        }
    
        return view('student.join_exam', compact('exam'));
    }
    ...
    public function submit_questions(Request $request){
        ...
        for($i = 1; $i <= $request->index; $i++) {
            if (isset($data['question' . $i])) {
                $q = Oex_question_master::where('id', $data['question' . $i])->first();
        ...
        return redirect()->route('exam')->with('success', 'Jawaban telah disimpan.');
    }    

    //Applying for exam
    public function apply_exam($id){
        $checkuser = user_exam::where('user_id', Session::get('id'))->where('exam_id', $id)->first();
    
        if ($checkuser) {
            $arr = ['status' => 'false', 'message' => 'Sudah terdaftar, lihat di bagian ujian Anda'];
        } else {
            ...
            $arr = ['status' => 'true', 'message' => 'Berhasil mendaftar', 'reload' => url('student/dashboard')];
        }
    ...
    public function view_result($id){
        $data['result_info'] = Oex_result::where('exam_id', $id)->where('user_id', Session::get('id'))->first();
        $data['student_info'] = User::where('id', Session::get('id'))->first();
        $data['exam_info'] = Oex_exam_master::where('id', $id)->first();
